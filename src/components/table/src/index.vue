...
// columns 处理
const getColumns = (checkbox: boolean, columns: DataTableColumns, actionsColumns: DataTableColumn): DataTableColumns => {
	let res = null
	if (checkbox) {
...
const scroll_x = computed(() => getScrollX(
	props.columns,
	props.scrollX,
...
<template>
	<div class="zai-table-view" :style="cssVars">
				...
				<n-tooltip trigger="hover">
					<template #trigger>
						<div class="flex-alc zt-header-icon_right view-curspointer">
							<n-dropdown trigger="click" :options="densityOptions" @select="densitySelect"
								:value="tableSize">
								<n-icon size="18">
									<ColumnHeightOutlined />
								</n-icon>
		...
		<n-data-table class="zai-table" ref="dataTableRef" flex-height :columns="tableColumns" :data="props.data"

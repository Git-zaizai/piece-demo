...
function move(e: TouchEvent) {
	...
	console.log(viewScrollTop, viewClientHeight)

	if (viewScrollTop - viewClientHeight <= 0 && distanceY > 0) {
...
function end() {
	...
	if (viewScrollTop === 0 || viewScrollTop <= viewClientHeight && distanceY > 0) {
		if (distanceY < DISTANCE_Y_MIN_LIMIT) {
			downRef.value = {
				iconStatus: 1,
				text: '下拉'
...
onMounted(() => {
	...
	if (pullDom) {
		...
		viewClientHeight = scrollYRef.value.clientHeight
		loadLock = true
		onDownMoutd()
	}
})

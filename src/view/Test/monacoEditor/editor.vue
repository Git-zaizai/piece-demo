...
import {
    ...
    FullscreenOutlined, CloseOutlined, MinusOutlined
} from "@vicons/antd"
...
watch(() => [props.show, modalshow.value], val => {
    modalshow.value = val[0]
    emit('update:show', val.at(-1))
    nextTick(() => {
        loader.init().then(monaco => {
            console.log(val.at(-1), editorViewRef.value, !monacoEditor);

            if (val.at(-1) && editorViewRef.value && !monacoEditor) {
                monacoEditor = monaco
                monaco.editor.create(editorViewRef.value, {
    ...
    <n-modal v-model:show="modalshow" :show-icon="false" display-directive="show" class="modal-editor-vscode"
        ...
        <div class="editor-vscode">
            <header class="editor-vscode-header w-100">
                <div class="editor-vscode-header-button">
                    <n-button-group>
